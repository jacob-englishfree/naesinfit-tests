...
## ⛔ 용어 매핑 — "퀴즈" = "예상문제" (2026-08-24 확정, 모든 세션 필독)

**테스트 3종 = 단어 / 워크북 / 예상문제.** 예전 명칭 "퀴즈"는 **화면 표시상 전면 폐기** → 학생·선생님·홈페이지·마케팅 등 사람이 보는 모든 곳은 "예상문제 테스트".

**단, 내부 코드/데이터는 `퀴즈` 그대로 유지** (리브랜딩 원칙 = 화면만 바꾸고 코드는 유지):
- 파일명 `data/**/퀴즈.json` (약 8,630개), `type:"퀴즈"`
- `create-test.js --type 퀴즈`, `testTypeMap.퀴즈=quizTest`, histKey `_v5`
- 앱 내부 키 `'퀴즈테스트'`(DB `test_type` 저장값), 영어키 `quiz`
- validate `TW-TYPE` 화이트리스트의 "퀴즈", 숙제분류 정규식 `/예상문제|퀴즈|.../`

즉 **검색 시**: 화면/문서에서 안 보이면 "퀴즈"로도 grep해볼 것(내부는 퀴즈로 남아있음). 출제 명령·파일 경로는 계속 `퀴즈` 사용.
...
- **19**: S-CHOICE-NOT-IN-PASSAGE: 내용일치/불일치 선지 고유명사·키워드가 passage에 없고 fullPassage에만 있음 — 발췌 잘림
