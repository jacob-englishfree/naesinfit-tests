...
const META_TOKEN_PATTERNS = [
  ...
  /✅/g,
  /❌/g,
  // PDF OCR 부산물: "6 영역 고 2", "영어 영역", "고2 6월" 등 시험지 헤더/푸터
  /\s*\d+\s*영역\s*고\s*\d+\s*/g,
  /\s*영어\s*영역\s*/g,
  /\s*고\s*\d+\s*\d{1,2}월\s*/g,
];
...
function fixFile(file) {
  ...
  questions.forEach((q, qi) => {
    ...
    const stripMeta = (s) => {
      ...
      for (const pat of META_TOKEN_PATTERNS) {
        r = r.replace(pat, '').replace(/\s+/g, ' ').trim();
      }
      return r;
    };
