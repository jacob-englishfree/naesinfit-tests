...
function validate(jsonPath) {
  ...
  questions.forEach((q, i) => {
    ...
    if (isWritten && wa && passage) {
      const isFindInPassage = /본문에서\s*찾아|원문에서\s*찾아|본문\s*속|발췌|본문 그대로|본문에서\s*골라|지문에서\s*찾아|글에서\s*찾아|찾아\s*쓰시오|원문\s*속/.test(stem);
      const isWriting = /영작/.test(typeNorm) || /영작/.test(stem);
  ...
  for (const q of questions) {
    ...
    if (/빈칸\s*어휘/.test(qtype) && fmt === 'mc' && ch.length > 0 && q.ans >= 1 && q.ans <= ch.length) {
      ...
      if (ansText && ansText.length >= 3 && fullPassage && !fullPassage.toLowerCase().includes(ansText.toLowerCase())) {
