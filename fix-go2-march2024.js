#!/usr/bin/env node
/**
 * Fix ALL test files for 고2 2024 3월
 *
 * 1. Apply 1강 reference UI template (slide mode)
 * 2. Add 5th choices where ch.length<5 (except T/F=2, 문장삽입=4)
 * 3. Fix \n → <br> in passages
 * 4. Fix T/F to have exactly 2 choices
 * 5. Verify total pts = 100
 * 6. Add html2canvas script
 */

const fs = require('fs');
const path = require('path');

const BASE = '/Users/woobumpark/Desktop/영어해방공식&내신핏/naesinfit-tests/모의고사/고2/3월_2024';
...
.q-header{display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(135deg,#F0FDF4,#E8F5E9);border-bottom:1px solid #e0ece2;flex-wrap:wrap;}
...
.tag-combo{background:var(--gxl);color:#166534;}.tag-wrong{background:var(--rl);color:var(--r);}.tag-blank{background:#E0F7FA;color:#00838F;}.tag-syn{background:var(--bl);color:var(--b);}.tag-ant{background:var(--ol);color:#B45309;}.tag-poly{background:#FCE4EC;color:#C2185B;}.tag-eng{background:#E8F6FF;color:#0277BD;}.tag-morph{background:#FFF3E0;color:#E65100;}.tag-ctx{background:#EDE7F6;color:#5E35B1;}
...
.tag-written{background:var(--gxl);color:var(--gd);}
...
.q-given-sentence{background:var(--gxl);border:2px solid var(--gl);border-radius:var(--rs);padding:14px 16px;margin-bottom:14px;font-size:13.5px;line-height:1.8;font-weight:600;}
...
.written-input{width:100%;padding:10px 12px;border:1.5px solid #dde8df;border-radius:8px;font-size:14px;font-family:inherit;}.written-input:focus{outline:none;border-color:var(--g);}
...
.tag-list{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;}.tag-item{padding:5px 12px;border-radius:16px;font-size:12px;font-weight:700;}.tag-item.good{background:var(--gxl);color:var(--g);}.tag-item.weak{background:var(--rl);color:var(--r);}
...
.expl-tag{display:inline-flex;font-size:11px;font-weight:800;padding:4px 10px;border-radius:6px;margin-bottom:6px;}.expl-tag.ans{background:var(--gxl);color:var(--g);}.expl-tag.tip{background:var(--gxl);color:var(--gd);}
...
const CSS_WORKBOOK = `:root{--o:#7C3AED;--od:#5B21B6;--ol:#A78BFA;--oxl:#F3F0FF;--oxxl:#FAF5FF;--bg:#F5F3FF;--text:#1A1A2E;--tm:#6B5B8A;--tl:#9A8DB8;--r:#D63B3B;--rl:#FFF0F0;--b:#3B6FD9;--bl:#EDF2FF;--warn:#F59E0B;--wl:#FFF8E1;--card:#FFF;--shadow:0 4px 20px rgba(0,0,0,.06);--radius:16px;--rs:10px;--rxs:6px;--tr:all .2s ease;}
*{margin:0;padding:0;box-sizing:border-box;}html{scroll-behavior:smooth;}
body{font-family:'Pretendard Variable',-apple-system,sans-serif;background:var(--bg);color:var(--text);line-height:1.7;-webkit-font-smoothing:antialiased;}
.screen{display:none;min-height:100vh;}.screen.active{display:flex;flex-direction:column;}
.toast{position:fixed;bottom:100px;left:50%;transform:translateX(-50%) translateY(80px);background:var(--text);color:#fff;padding:12px 24px;border-radius:var(--rs);font-size:14px;font-weight:600;opacity:0;transition:all .3s;z-index:999;pointer-events:none;}.toast.show{opacity:1;transform:translateX(-50%) translateY(0);}
...
.info-label{color:var(--tm);}.info-value{font-weight:700;}
.chip-row{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:26px;}
.chip{background:var(--card);border:1.5px solid #d8d0e8;border-radius:24px;padding:7px 15px;font-size:12px;font-weight:600;color:var(--tm);}
.input-group{margin-bottom:14px;}.input-group label{display:block;font-size:12px;font-weight:700;color:var(--tm);margin-bottom:6px;}
...
.q-card{background:var(--card);border-radius:12px;margin-bottom:14px;box-shadow:var(--shadow);overflow:hidden;}
.q-header{display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(135deg,#F3F0FF,#E8E0FF);border-bottom:1px solid #e0d8ec;flex-wrap:wrap;}
...
.q-type-tag{font-size:11px;font-weight:800;padding:4px 12px;border-radius:14px;}
.tag-combo{background:var(--oxl);color:#5E35B1;}.tag-wrong{background:var(--rl);color:var(--r);}.tag-blank{background:#E0F7FA;color:#00838F;}.tag-syn{background:var(--bl);color:var(--b);}.tag-ant{background:var(--ol);color:#B45309;}.tag-poly{background:#FCE4EC;color:#C2185B;}.tag-eng{background:#E8F6FF;color:#0277BD;}.tag-morph{background:#FFF3E0;color:#E65100;}.tag-ctx{background:#EDE7F6;color:#5E35B1;}
.tag-insert{background:#E0F7FA;color:#00838F;}
.tag-order{background:#FFF3E0;color:#E65100;}
.tag-grammar{background:#EDE7F6;color:#5E35B1;}
.tag-vocab{background:#FCE4EC;color:#C2185B;}
...
.q-body{padding:14px 16px;}
.q-passage{background:linear-gradient(135deg,#FAFBFF,#F5F0FF);border-left:3px solid var(--o);padding:12px 14px;margin-bottom:12px;border-radius:0 8px 8px 0;line-height:1.85;text-align:justify;word-break:keep-all;overflow-wrap:break-word;font-size:clamp(13px,3.2vw,15px);}
.q-stem{font-size:13px;font-weight:700;margin-bottom:10px;overflow-wrap:break-word;}
...
.choices{display:flex;flex-direction:column;gap:4px;}
.choice-btn{display:flex;align-items:center;gap:8px;padding:8px 10px;border:1.5px solid #d8d0e8;border-radius:8px;cursor:pointer;transition:var(--tr);background:#fff;text-align:left;font-family:inherit;font-size:13px;line-height:1.4;}
...
.written-input{width:100%;padding:10px 12px;border:1.5px solid #d8d0e8;border-radius:8px;font-size:14px;font-family:inherit;}.written-input:focus{outline:none;border-color:var(--o);}
.exam-nav{position:fixed;bottom:0;left:0;right:0;background:rgba(255,255,255,.95);backdrop-filter:blur(12px);border-top:1px solid rgba(0,0,0,.06);padding:10px 16px;display:flex;align-items:center;gap:5px;justify-content:center;z-index:100;overflow-x:auto;flex-wrap:wrap;}
...
.r-hero{padding:48px 20px 36px;text-align:center;}.r-hero.pass{background:linear-gradient(160deg,#E8E0FF,#C8E6C9);}.r-hero.fail{background:linear-gradient(160deg,#FFF3E8,#FFEAEA);}
...
.level-bar{margin-top:16px;height:8px;background:#e8e0f0;border-radius:4px;overflow:hidden;}.level-bar-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,var(--o),#A78BFA);transition:width 1.5s ease;}.level-msg{margin-top:12px;font-size:14px;color:var(--tm);}
...
const CSS_QUIZ = `:root{--o:#E8772E;--od:#B85A1A;--ol:#F5A623;--oxl:#FFF3E8;--oxxl:#FFFAF5;--bg:#F6F2EE;--text:#1A1A2E;--tm:#7A6A5E;--tl:#A89A8E;--r:#D63B3B;--rl:#FFF0F0;--b:#3B6FD9;--bl:#EDF2FF;--warn:#F59E0B;--wl:#FFF8E1;--card:#FFF;--shadow:0 4px 20px rgba(0,0,0,.06);--radius:16px;--rs:10px;--rxs:6px;--tr:all .2s ease;}
*{margin:0;padding:0;box-sizing:border-box;}html{scroll-behavior:smooth;}
body{font-family:'Pretendard Variable',-apple-system,sans-serif;background:var(--bg);color:var(--text);line-height:1.7;-webkit-font-smoothing:antialiased;}
.screen{display:none;min-height:100vh;}.screen.active{display:flex;flex-direction:column;}
.toast{position:fixed;bottom:100px;left:50%;transform:translateX(-50%) translateY(80px);background:var(--text);color:#fff;padding:12px 24px;border-radius:var(--rs);font-size:14px;font-weight:600;opacity:0;transition:all .3s;z-index:999;pointer-events:none;}.toast.show{opacity:1;transform:translateX(-50%) translateY(0);}
...
.tag-content{background:#E8F5E9;color:#2E7D32;}
.tag-tf{background:#FFF8E1;color:#F57F17;}
.tag-combo{background:var(--oxl);color:#E65100;}.tag-wrong{background:var(--rl);color:var(--r);}.tag-syn{background:var(--bl);color:var(--b);}.tag-ant{background:var(--ol);color:#B45309;}.tag-poly{background:#FCE4EC;color:#C2185B;}.tag-eng{background:#E8F6FF;color:#0277BD;}.tag-morph{background:#FFF3E0;color:#E65100;}.tag-ctx{background:#EDE7F6;color:#5E35B1;}
...
// ==========================================
// JS Template (new slide-based from 1강 reference)
// Replaces everything from startExam to end of script
// ==========================================
...
function getTypeAdvice(t){const m={'(A)(B)(C) 조합형':'반의어 함정 주의, 원문 단어 정확히 암기','문맥상 부적절한 어휘':'밑줄 단어가 반의어로 바뀌었는지 확인','동의어 고르기':'동의어 쌍을 세트로 묶어 암기','반의어 고르기':'반의어 쌍을 세트로 묶어 암기','빈칸 어휘 완성':'문맥 단서(clue) 찾기 연습 필요','다의어 문맥적 의미':'다의어의 여러 뜻을 문맥별로 구분','영영풀이 매칭':'영영사전 정의 읽기 연습','어형 변환 (서술형)':'품사 변환 규칙(명↔형, 동↔명) 정리','빈칸 문맥 완성':'글의 흐름과 논리적 연결 파악','어법':'핵심 어법 규칙 정리 및 반복 연습','내용일치':'지문과 선지를 꼼꼼히 대조','내용불일치':'함정 선지의 미세한 차이 찾기','T/F':'원문과 진술을 정확히 대조','서술형':'정확한 철자와 어형 변환 주의','빈칸추론':'빈칸 전후 문맥 단서 찾기','빈칸 추론':'빈칸 전후 문맥 단서 찾기','문장삽입':'글의 흐름과 연결사 파악','어순배열':'문장 구조(S+V+O) 정확히 파악','어휘':'문맥에서 적절한 어휘 고르기 연습','내용이해':'지문 핵심 내용 정확히 파악'};return m[t]||'해당 유형 집중 복습 필요';}
...
function extractData(html) {
  // Extract EI
  const eiMatch = html.match(/const EI\s*=\s*(\{[^}]+\})/);
  if (!eiMatch) return null;

  // Extract FULL_PASSAGE if exists
  const fpMatch = html.match(/const FULL_PASSAGE\s*=\s*`([^`]*)`/);

  // Extract Q array - find from "const Q=[" to the matching "];"
  const qStart = html.indexOf('const Q=[');
  if (qStart === -1) return null;

  // Find the end - we need to find the matching ];
  let depth = 0;
  let inString = false;
  let stringChar = '';
  let qEnd = -1;
  ...
  const qArrayStr = html.substring(html.indexOf('[', qStart), qEnd);

  // Extract any passage constants (FULL_PASSAGE, P_XXX, etc.)
  const passageConsts = [];
  const constRegex = /const\s+(FULL_PASSAGE|P_\w+)\s*=\s*`([^`]*)`/g;
  let m;
  while ((m = constRegex.exec(html)) !== null) {
    passageConsts.push({ name: m[1], value: m[2] });
  }

  // Extract typeTag if exists
  const typeTagMatch = html.match(/const typeTag\s*=\s*(\{[^}]+\})/);

  // Get everything between const Q=[ ... ]; including any constants before Q
  const scriptStart = html.indexOf('<script>');
  const scriptEnd = html.indexOf('</script>');
  const scriptContent = html.substring(scriptStart + 8, scriptEnd);

  // Extract EI line
  const eiLine = scriptContent.match(/const EI\s*=.+?;/)[0];

  // Extract passage constants section
  const passageSection = passageConsts.map(p => `const ${p.name}=\`${p.value}\`;`).join('\n\n');

  // Extract Q array section
  const qSection = html.substring(html.indexOf('const Q='), qEnd) + ';';

  // Extract typeTag
  let typeTagLine = '';
  if (typeTagMatch) {
    typeTagLine = `const typeTag=${typeTagMatch[1]};`;
  }

  return { eiLine, passageSection, qSection, typeTagLine, eiMatch: eiMatch[1] };
}

// ==========================================
// Fix Q array data issues
// ==========================================

function fixQData(qSection) {
  let fixed = qSection;

  // Fix \n → <br> in passages (literal \n in template literals)
  // We need to replace actual newlines inside backtick strings
  // This is tricky - the passages use \n as literal characters in regular strings
  // or actual newlines in template literals

  // Replace literal \n (not inside <br> context) with <br>
  // In the file, passages have real newlines that should be <br>
  fixed = fixed.replace(/(`[^`]*`)/g, (match) => {
    return match.replace(/\n/g, '<br>');
  });

  // Also fix regular string passages with \n
  fixed = fixed.replace(/(["'][^"']*["'])/g, (match) => {
    // Only in passage contexts - check if it's a long string with \n
    if (match.includes('\\n') && match.length > 100) {
      return match.replace(/\\n/g, '<br>');
    }
    return match;
...
function buildHTML(type, folderName, data) {
  const testType = type === 'word' ? '단어' : type === 'workbook' ? '워크북' : '퀴즈';
  const testLabel = type === 'word' ? '온라인단어 TEST' : type === 'workbook' ? '온라인워크북 TEST' : '온라인퀴즈 TEST';
  const brandLabel = `📝 ${testLabel}`;
  const css = type === 'word' ? CSS_WORD : type === 'workbook' ? CSS_WORKBOOK : CSS_QUIZ;

  // Parse EI to get info
  let ei;
  try {
    ei = JSON.parse(data.eiMatch.replace(/'/g, '"'));
  } catch(e) {
    // Try eval
    try {
      ei = eval('(' + data.eiMatch + ')');
    } catch(e2) {
      ei = { subject: '2024 고2 3월 모의고사', pub: folderName, lesson: '', title: '', total: 100, time: 1200, totalQ: 20 };
    }
  }

  // Determine typeTag based on test type
  let typeTagConst;
  if (data.typeTagLine) {
    typeTagConst = data.typeTagLine;
  } else {
    // Use full typeTag from quiz reference
    typeTagConst = `const typeTag={'문장삽입':'tag-insert','어순배열':'tag-order','어법':'tag-grammar','어휘':'tag-vocab','빈칸추론':'tag-blank','서술형':'tag-written','내용이해':'tag-content','T/F':'tag-tf','(A)(B)(C) 조합형':'tag-combo','문맥상 부적절한 어휘':'tag-wrong','동의어 고르기':'tag-syn','반의어 고르기':'tag-ant','다의어 문맥적 의미':'tag-poly','영영풀이 매칭':'tag-eng','어형 변환 (서술형)':'tag-morph','빈칸 어휘 완성':'tag-ctx','빈칸 문맥 완성':'tag-ctx','내용일치':'tag-content','내용불일치':'tag-content','빈칸 추론':'tag-blank'};`;
  }

  const fixedQSection = fixQData(data.qSection);
...
<title>2024 고2 3월 ${folderName} ${testLabel}</title>
...
<div class="start-container">
  ...
  <h1 class="start-title">${testLabel.replace(' TEST', '')} <span>TEST</span></h1>
  <p class="start-subtitle">${ei.subject} · ${ei.pub} · ${ei.lesson || ''} · ${ei.totalQ || 20}문항</p>
  ...
  <div class="chip-row"><div class="chip">📝 ${ei.totalQ || 20}문항</div><div class="chip">⏱️ ${Math.floor((ei.time||1200)/60)}분</div><div class="chip">✅ 80점 이상 통과</div><div class="chip">📊 100점 만점</div></div>
...
<div id="examScreen" class="screen">
  <div class="exam-topbar"><div class="topbar-left"><div class="topbar-dot"></div><div class="topbar-info">2024 고2 3월 · <span>${ei.pub}</span></div></div><div class="timer" id="timer">${Math.floor((ei.time||1200)/60)}:00</div><button class="btn-submit" onclick="submitExam()">제출하기</button></div>
...
const testTypes = [
  { file: '단어테스트.html', type: 'word' },
  { file: '워크북테스트.html', type: 'workbook' },
  { file: '퀴즈테스트.html', type: 'quiz' }
];
...
for (const folder of folders) {
  ...
  for (const { file, type } of testTypes) {
    ...
    try {
      ...
      if (!data) {
        errors.push(`PARSE_FAIL: ${folder}/${file} - could not extract data`);
        continue;
      }

      const newHTML = buildHTML(type, folder, data);
      fs.writeFileSync(filePath, newHTML, 'utf8');
      totalFixed++;
      console.log(`  ✅ ${folder}/${file}`);
    } catch (e) {
      errors.push(`ERROR: ${folder}/${file} - ${e.message}`);
    }
  }
}
