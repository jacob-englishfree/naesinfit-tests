#!/usr/bin/env node
/**
 * Fix ALL test files for 고2 2025 3월
...
const BASE = path.join(__dirname, '모의고사/고2/3월');
...
function addFifthChoice(html) {
  ...
  return html.replace(/ch:\[([^\]]*)\]/g, (match, inner) => {
    // Count backtick-delimited items
    const items = inner.match(/`[^`]*`/g) || [];
    const count = items.length;

    // Skip: T/F (2), 문장삽입 (4 = ①②③④), written (0), already 5
    ...
    // For 문장삽입 with ①②③④ - keep as 4 (these are insertion points)
    // Already handled above by the ① check

    // For 4 English/Korean word choices - add a 5th distractor
    if (count === 4) {
      // Parse existing choices
      const trimmed = inner.trim();
      // Add a generic 5th choice based on context
      return match.slice(0, -1) + addDistractor(items) + ']';
    }

    if (count === 3) {
      // Rare case - add 2 more
      return match.slice(0, -1) + addDistractor(items) + addDistractor(items, true) + ']';
    }
...
function addDistractor(items, second = false) {
  // Try to infer what kind of choices these are
  const first = items[0].replace(/`/g, '');

  // Korean choices
  if (/[\uAC00-\uD7AF]/.test(first)) {
    if (second) return ',`위 내용과 관련 없는 선지`';
    return ',`위의 어느 것도 해당하지 않는다.`';
  }

  // English word choices
  if (second) return ',`conclude`';
  return ',`eliminate`';
}
...
function buildQuizHTML(origHTML, dirName) {
  ...
  let ei;
  try { ei = JSON.parse(eiMatch[1]); } catch(e) { return origHTML; }

  // Extract everything from const EI to end of Q array
  const dataStart = dataSection.indexOf('const EI=');
  const qEnd = dataSection.indexOf('\n];\n', dataSection.indexOf('const Q=['));
...
<div class="start-container">
  ...
  <div class="input-row">
    ...
    <div class="input-group" style="flex:.6"><label>학년</label><select id="inpGrade"><option>고1</option><option selected>고2</option><option>고3</option><option>중3</option></select></div>
  </div>
...
<div id="examScreen" class="screen">
  <div class="exam-topbar"><div class="topbar-left"><div class="topbar-dot"></div><div class="topbar-info">${ei.subject.replace('2025 고2 3월 모의고사','2025 고2 3월')} · <span>${ei.pub}</span></div></div><div class="timer" id="timer">20:00</div><button class="btn-submit" onclick="submitExam()">제출하기</button></div>
...
function reviewHistory(idx){const h=getHistory();const rec=h[idx];if(!rec||!rec.answers)return toast('이전 버전 기록은 해설을 볼 수 없습니다.');S.ans=rec.answers;S.start=Date.now()-rec.elapsed*1000;document.getElementById('inpName').value=rec.name||'';document.getElementById('inpSchool').value=rec.school||'';document.getElementById('inpGrade').value=rec.grade||'고2';const r=grade();renderResults(r,rec.elapsed);showScreen('resultScreen');window.scrollTo(0,0);}
...
function buildScrollHTML(origHTML, testType, dirName) {
  ...
  const topInfo = `${ei.subject.replace('2025 고2 3월 모의고사','2025 고2 3월')} · <span>${ei.pub}</span>`;
