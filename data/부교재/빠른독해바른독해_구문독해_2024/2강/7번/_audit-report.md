# Audit Report: 빠른독해바른독해 구문독해 2과 7번

## 테스트 현황

| 테스트 | 문항 | 총점 | validate | blind | adversarial |
|--------|------|------|----------|-------|-------------|
| 단어 | 20 | 100 | PASS (10 warnings) | - | PASS (0 issues) |
...
- EX-1: Q9 빈칸 정답 "symptoms"가 지문에 노출 -- 빈칸 위치의 문맥 판단이 핵심이므로 유효
- C20: histKey 패턴 불일치 (기능 영향 없음)
- P2 x5: ABC(Q1)/부적절어휘(Q4)/다의어(Q15)/어형변환(Q17-18) passage가 fullPassage 외 별도 작성 -- 유형 특성상 정상

### 워크북
- EX-2 x2: Q15/Q16 서술형 wa가 지문에 노출 -- 찾기 유형(stem에 "본문에서 찾아")이므로 정당
- C20: histKey 패턴 불일치
- P2 x2: 어법(Q2)/오류찾기(Q14) passage 마커 삽입으로 시작 위치 차이 -- 정상

### 퀴즈
- EX-2: Q17 서술형 wa가 지문에 노출 -- 찾기 유형이므로 정당
- C20: histKey 패턴 불일치
- T39: Q18 어순배열 그룹 배치 권고 -- 기능 영향 없음
- P2: Q4 passage 마커 삽입으로 시작 위치 차이 -- 정상
