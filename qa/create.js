#!/usr/bin/env node
/**
 * create.js — 내신해방공식 테스트 통합 출제 파이프라인
 *
...
## ⛔ passage & 정답 노출 원칙 (2026-04-06 확정 · S급)

모의고사·교과서·부교재 공통. 위반 시 폐기 + 재출제.

**A. passage 필수**
- 모든 mc + 서술형(written) 문항에 passage 필수.
- 예외: 영영풀이, 다의어 (A)(B) 미니 문장.
- 한영 번역도 passage 필수 (영어 정답 자리만 __________).

**B. 정답 노출 — 유형별**
- 빈칸(빈칸추론/빈칸어휘/빈칸문맥/연결사): 본문 흐름 유지 + 빈칸 자리만 __________. 본문 다른 곳에 정답 단어가 등장해도 OK. 단, 빈칸 직후 5단어 이내 정답 그대로 노출 금지.
- 본문에서 찾기: 정답이 passage에 그대로 있어도 OK. stem에 "본문에서 찾아 쓰시오" 명시 필수.
- 함축의미 추론: 밑줄 구절은 원문 그대로 OK.
- 어법/어휘 부적절: 밑줄 단어는 원문 그대로 OK.
- 내용일치/주제/제목 (한국어 선지): 본문 직역 금지, 반드시 패러프레이즈.
- 한영 번역: passage 있되 영어 정답 자리만 __________.

**C. passage 길이**
- 원칙: 5문장 이상 발췌.
- 예외: 짧은 지문(안내문/광고문/일기/대화문)은 fullPassage 전체 OK.

**D. 적용 범위**
- 모의고사 + 교과서 + 부교재 공통. 유형/규칙/검수 기준 동일.

## ⛔ 빈칸 출제 단어 선택 원칙 (2026-04-08)
...
- 형용사
- 핵심 명사 (주제어)
- 연결사 (however, therefore 등)

❌ 출제 금지 (단순 암기형):
- 숫자 (날짜, 가격, 시간)
- 고유명사 (사람 이름, 지명, 회사명)
- 특정 사실 (요일, 색깔 등 본문 외 단서 0인 것)

판단 기준: 학생이 본문 단서만으로 답을 떠올릴 수 있는가? 떠올릴 수 없으면 단순 암기 → 폐기.

## ⛔ 영작 [조건] 모든 단어 명시 (2026-04-08, S-COND-WORD-MATCH)

서술형 영작 stem의 [조건]에 wa(정답)의 모든 단어를 빠짐없이 명시. 기능어(a, an, the, and, or, to, is, are 등)도 빠짐없이.
...
규칙:
- wa의 모든 단어가 [조건] (1)에 명시되어야 함
- 기능어(a, an, the, and, or, to, of, is, are 등) 빠짐없이
