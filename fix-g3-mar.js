#!/usr/bin/env node
/**
 * Fix ALL test files for 고3 2025 3월
 * 1. Add 5th choices where ch.length<5 (except T/F=2, 문장삽입=4, 서술형)
 * 2. Apply 1강 UI template (slide view for quiz, html2canvas)
 * 3. Fix \n → <br> in stems
 * 4. Verify total pts = 100
 */

const fs = require('fs');
const path = require('path');

const BASE = path.join(__dirname, '모의고사/고3/3월');
...
const QUIZ_CSS = `:root{--o:#E8772E;--od:#B85A1A;--ol:#F5A623;--oxl:#FFF3E8;--oxxl:#FFFAF5;--bg:#F6F2EE;--text:#1A1A2E;--tm:#7A6A5E;--tl:#A89A8E;--r:#D63B3B;--rl:#FFF0F0;--b:#3B6FD9;--bl:#EDF2FF;--warn:#F59E0B;--wl:#FFF8E1;--card:#FFF;--shadow:0 4px 20px rgba(0,0,0,.06);--radius:16px;--rs:10px;--rxs:6px;--tr:all .2s ease;}
...
.expl-box{font-size:13.5px;line-height:1.85;padding:14px 16px;background:#FAFAF8;border-radius:var(--rxs);margin-bottom:10px;}.expl-box.ans-b{border-left:3px solid var(--o);}.expl-box.tip-b{border-left:3px solid var(--ol);background:var(--oxl);}
...
const WORD_CSS = `:root{--g:#16A34A;--gd:#14532D;--gl:#E8F5E9;--gxl:#F0FDF4;--gxxl:#F8FDF9;--bg:#F4F8F5;--text:#1A1A2E;--tm:#5A6A5E;--tl:#8A9A8E;--r:#D63B3B;--rl:#FFF0F0;--b:#3B6FD9;--bl:#EDF2FF;--o:#F59E0B;--ol:#FFF8E1;--card:#FFF;--shadow:0 4px 20px rgba(0,0,0,.06);--radius:16px;--rs:10px;--rxs:6px;--tr:all .2s ease;}
*{margin:0;padding:0;box-sizing:border-box;}html{scroll-behavior:smooth;}
body{font-family:'Pretendard Variable',-apple-system,sans-serif;background:var(--bg);color:var(--text);line-height:1.7;-webkit-font-smoothing:antialiased;}
.screen{display:none;min-height:100vh;}.screen.active{display:flex;flex-direction:column;}
.toast{position:fixed;bottom:100px;left:50%;transform:translateX(-50%) translateY(80px);background:var(--text);color:#fff;padding:12px 24px;border-radius:var(--rs);font-size:14px;font-weight:600;opacity:0;transition:all .3s;z-index:999;pointer-events:none;}.toast.show{opacity:1;transform:translateX(-50%) translateY(0);}
...
.info-label{color:var(--tm);}.info-value{font-weight:700;}
.chip-row{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:26px;}.chip{background:var(--card);border:1.5px solid #dde8df;border-radius:24px;padding:7px 15px;font-size:12px;font-weight:600;color:var(--tm);}
.input-group{margin-bottom:14px;}.input-group label{display:block;font-size:12px;font-weight:700;color:var(--tm);margin-bottom:6px;}
...
.exam-scroll{max-width:820px;margin:0 auto;padding:24px 20px 100px;}
.q-card{background:var(--card);border-radius:var(--radius);margin-bottom:22px;box-shadow:var(--shadow);overflow:hidden;}
.q-header{display:flex;align-items:center;gap:10px;padding:16px 22px;background:linear-gradient(135deg,#F0FDF4,#E8F5E9);border-bottom:1px solid #e0ece2;flex-wrap:wrap;}
.q-num{width:36px;height:36px;background:linear-gradient(135deg,var(--gd),var(--g));color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:900;flex-shrink:0;}
.q-type-tag{font-size:11px;font-weight:800;padding:4px 12px;border-radius:14px;}
.tag-combo{background:var(--gl);color:#166534;}.tag-wrong{background:var(--rl);color:var(--r);}.tag-blank{background:#E0F7FA;color:#00838F;}.tag-syn{background:var(--bl);color:var(--b);}.tag-ant{background:var(--ol);color:#B45309;}.tag-poly{background:#FCE4EC;color:#C2185B;}.tag-eng{background:#E8F6FF;color:#0277BD;}.tag-morph{background:#FFF3E0;color:#E65100;}.tag-ctx{background:#EDE7F6;color:#5E35B1;}
...
.q-body{padding:22px;}
.q-passage{background:linear-gradient(135deg,#FAFDFB,#F5FAF6);border-left:4px solid var(--g);padding:16px 18px;margin-bottom:18px;border-radius:0 var(--rs) var(--rs) 0;font-size:13.5px;line-height:1.95;}
.q-stem{font-size:14px;font-weight:700;margin-bottom:16px;}
.choices{display:flex;flex-direction:column;gap:8px;}
.choice-btn{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;border:2px solid #dde8df;border-radius:var(--rs);cursor:pointer;transition:var(--tr);background:#fff;text-align:left;font-family:inherit;font-size:14px;line-height:1.6;}
.choice-btn:hover{border-color:var(--g);}.choice-btn.selected{border-color:var(--g);box-shadow:0 0 0 3px rgba(22,163,74,.12);background:var(--gxxl);}
.c-num{min-width:26px;height:26px;background:#e8efe9;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:var(--tm);flex-shrink:0;}.choice-btn.selected .c-num{background:linear-gradient(135deg,var(--gd),var(--g));color:#fff;}
.written-input{width:100%;padding:14px 16px;border:2px solid #d0dcd2;border-radius:var(--rs);font-size:15px;font-family:inherit;}.written-input:focus{outline:none;border-color:var(--g);}
.exam-nav{position:fixed;bottom:0;left:0;right:0;background:rgba(255,255,255,.95);backdrop-filter:blur(12px);border-top:1px solid rgba(0,0,0,.06);padding:10px 16px;display:flex;align-items:center;gap:5px;justify-content:center;z-index:100;overflow-x:auto;flex-wrap:wrap;}
...
.r-hero{padding:48px 20px 36px;text-align:center;}.r-hero.pass{background:linear-gradient(160deg,#E8F5E9,#C8E6C9);}.r-hero.fail{background:linear-gradient(160deg,#FFF3E8,#FFEAEA);}
.r-badge{display:inline-block;padding:8px 20px;border-radius:24px;font-size:14px;font-weight:800;margin-bottom:16px;}.r-badge.pass{background:linear-gradient(135deg,var(--g),#15A35A);color:#fff;}.r-badge.fail{background:linear-gradient(135deg,var(--r),#C42A2A);color:#fff;}
.r-score{font-size:72px;font-weight:900;}.r-score sup{font-size:24px;color:var(--tl);}
.r-stats{display:flex;justify-content:center;gap:14px;margin-top:16px;font-size:13px;color:var(--tm);flex-wrap:wrap;}.r-stats strong{color:var(--text);font-weight:800;}.stat-item{background:rgba(255,255,255,.6);padding:6px 14px;border-radius:20px;}
.rc{max-width:740px;margin:0 auto;padding:0 20px 50px;}.rs{background:var(--card);border-radius:var(--radius);padding:24px;margin-bottom:18px;box-shadow:var(--shadow);}.rs-title{font-size:17px;font-weight:800;margin-bottom:18px;}
.level-card{text-align:center;padding:30px 20px;}.level-icon{font-size:64px;margin-bottom:8px;animation:floatY 3s ease-in-out infinite;}@keyframes floatY{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
...
.level-bar{margin-top:16px;height:8px;background:#e8efe9;border-radius:4px;overflow:hidden;}.level-bar-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,var(--g),#4ADE80);transition:width 1.5s ease;}.level-msg{margin-top:12px;font-size:14px;color:var(--tm);}
.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;}.info-cell{padding:12px;background:#FAFDFB;border-radius:var(--rs);border:1px solid #dde8df;}.info-cell .ic-label{font-size:11px;color:var(--tl);font-weight:700;margin-bottom:2px;}.info-cell .ic-value{font-size:15px;font-weight:800;}.info-cell.highlight{background:var(--gxl);}.info-cell.highlight .ic-value{color:var(--g);}
...
.rev-card{border:1.5px solid #dde8df;border-radius:var(--rs);margin-bottom:14px;overflow:hidden;}.rev-header{padding:14px 18px;background:#FAFDFB;display:flex;align-items:center;gap:12px;cursor:pointer;}.rev-header:hover{background:#F0F8F1;}.rev-icon{font-size:18px;margin-left:auto;transition:transform .2s;}.rev-body{max-height:0;overflow:hidden;transition:max-height .4s ease,padding .3s ease;padding:0 18px;}.rev-body.open{max-height:3000px;padding:20px 18px;}
...
const WORKBOOK_CSS = `:root{--o:#7C3AED;--od:#5B21B6;--ol:#A78BFA;--oxl:#F3F0FF;--oxxl:#FAF5FF;--bg:#F5F3FF;--text:#1A1A2E;--tm:#6B5B8A;--tl:#9A8DB8;--r:#D63B3B;--rl:#FFF0F0;--b:#3B6FD9;--bl:#EDF2FF;--warn:#F59E0B;--wl:#FFF8E1;--card:#FFF;--shadow:0 4px 20px rgba(0,0,0,.06);--radius:16px;--rs:10px;--rxs:6px;--tr:all .2s ease;}
*{margin:0;padding:0;box-sizing:border-box;}html{scroll-behavior:smooth;}
body{font-family:'Pretendard Variable',-apple-system,sans-serif;background:var(--bg);color:var(--text);line-height:1.7;-webkit-font-smoothing:antialiased;}
.screen{display:none;min-height:100vh;}.screen.active{display:flex;flex-direction:column;}
.toast{position:fixed;bottom:100px;left:50%;transform:translateX(-50%) translateY(80px);background:var(--text);color:#fff;padding:12px 24px;border-radius:var(--rs);font-size:14px;font-weight:600;opacity:0;transition:all .3s;z-index:999;pointer-events:none;}.toast.show{opacity:1;transform:translateX(-50%) translateY(0);}
...
.info-rows{padding:4px 0;}.info-row{display:flex;justify-content:space-between;padding:11px 20px;font-size:14px;border-bottom:1px solid #f0edf5;}.info-row:last-child{border:none;}
.info-label{color:var(--tm);}.info-value{font-weight:700;}
.chip-row{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:26px;}.chip{background:var(--card);border:1.5px solid #d8d0e8;border-radius:24px;padding:7px 15px;font-size:12px;font-weight:600;color:var(--tm);}
.input-group{margin-bottom:14px;}.input-group label{display:block;font-size:12px;font-weight:700;color:var(--tm);margin-bottom:6px;}
.input-group input,.input-group select{width:100%;padding:13px 16px;border:2px solid #d0c8dc;border-radius:var(--rs);font-size:15px;font-family:inherit;background:#fff;transition:var(--tr);}
...
.btn-outline{background:transparent;border:2px solid #c8c0d8;color:var(--tm);margin-top:8px;font-size:14px;padding:12px;}.btn-outline:hover{border-color:var(--o);color:var(--o);}
.history-badge{background:var(--oxl);color:var(--o);font-weight:800;padding:2px 8px;border-radius:10px;font-size:11px;margin-left:6px;}
#historyPanel{display:none;margin-top:14px;}
.history-item{background:var(--card);border:1.5px solid #d8d0e8;border-radius:var(--rs);padding:12px 14px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;}
.hi-left .hi-name{font-weight:700;font-size:13px;}.hi-left .hi-meta{font-size:11px;color:var(--tl);}
.hi-right .hi-score{font-size:20px;font-weight:900;color:var(--o);}.hi-right .hi-level{font-size:10px;color:var(--tm);text-align:right;}
.exam-topbar{position:sticky;top:0;z-index:100;background:rgba(255,255,255,.92);backdrop-filter:blur(16px);border-bottom:1px solid rgba(0,0,0,.06);padding:12px 24px;display:flex;align-items:center;justify-content:space-between;}
.topbar-left{display:flex;align-items:center;gap:10px;}.topbar-dot{width:8px;height:8px;background:var(--o);border-radius:50%;}
...
.exam-scroll{max-width:820px;margin:0 auto;padding:24px 20px 100px;}
.q-card{background:var(--card);border-radius:var(--radius);margin-bottom:22px;box-shadow:var(--shadow);overflow:hidden;}
.q-header{display:flex;align-items:center;gap:10px;padding:16px 22px;background:linear-gradient(135deg,#F3F0FF,#E8E0FF);border-bottom:1px solid #e0d8ec;flex-wrap:wrap;}
.q-num{width:36px;height:36px;background:linear-gradient(135deg,var(--od),var(--o));color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:900;flex-shrink:0;}
.q-type-tag{font-size:11px;font-weight:800;padding:4px 12px;border-radius:14px;}
.tag-grammar{background:#EDE7F6;color:#5E35B1;}.tag-content{background:#E0F7FA;color:#00838F;}.tag-blank{background:var(--bl);color:var(--b);}.tag-written{background:#FFF3E0;color:#E65100;}.tag-tf{background:#FCE4EC;color:#C2185B;}
.q-meta{display:flex;align-items:center;gap:8px;margin-left:auto;}.q-diff{font-size:11px;color:var(--tl);font-weight:600;background:#f0edf5;padding:3px 10px;border-radius:10px;}.q-pts{font-size:12px;color:var(--o);font-weight:800;}
.q-body{padding:22px;}
.q-passage{background:linear-gradient(135deg,#FAFBFF,#F5F0FF);border-left:4px solid var(--o);padding:16px 18px;margin-bottom:18px;border-radius:0 var(--rs) var(--rs) 0;font-size:13.5px;line-height:1.95;}
.q-stem{font-size:14px;font-weight:700;margin-bottom:16px;}
.choices{display:flex;flex-direction:column;gap:8px;}
.choice-btn{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;border:2px solid #d8d0e8;border-radius:var(--rs);cursor:pointer;transition:var(--tr);background:#fff;text-align:left;font-family:inherit;font-size:14px;line-height:1.6;}
.choice-btn:hover{border-color:var(--o);}.choice-btn.selected{border-color:var(--o);box-shadow:0 0 0 3px rgba(124,58,237,.12);background:var(--oxxl);}
.c-num{min-width:26px;height:26px;background:#e8e0f0;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:var(--tm);flex-shrink:0;}.choice-btn.selected .c-num{background:linear-gradient(135deg,var(--od),var(--o));color:#fff;}
.written-input{width:100%;padding:14px 16px;border:2px solid #d0c8dc;border-radius:var(--rs);font-size:15px;font-family:inherit;}.written-input:focus{outline:none;border-color:var(--o);}
.exam-nav{position:fixed;bottom:0;left:0;right:0;background:rgba(255,255,255,.95);backdrop-filter:blur(12px);border-top:1px solid rgba(0,0,0,.06);padding:10px 16px;display:flex;align-items:center;gap:5px;justify-content:center;z-index:100;overflow-x:auto;flex-wrap:wrap;}
.nav-dot{width:28px;height:28px;border-radius:50%;background:#d8d0e8;cursor:pointer;border:none;font-size:10px;font-weight:800;color:var(--tm);display:flex;align-items:center;justify-content:center;font-family:inherit;flex-shrink:0;}.nav-dot.answered{background:var(--o);color:#fff;}
.r-hero{padding:48px 20px 36px;text-align:center;}.r-hero.pass{background:linear-gradient(160deg,#EDE7F6,#D1C4E9);}.r-hero.fail{background:linear-gradient(160deg,#FFF3E8,#FFEAEA);}
.r-badge{display:inline-block;padding:8px 20px;border-radius:24px;font-size:14px;font-weight:800;margin-bottom:16px;}.r-badge.pass{background:linear-gradient(135deg,var(--o),#7C3AED);color:#fff;}.r-badge.fail{background:linear-gradient(135deg,var(--r),#C42A2A);color:#fff;}
.r-score{font-size:72px;font-weight:900;}.r-score sup{font-size:24px;color:var(--tl);}
.r-stats{display:flex;justify-content:center;gap:14px;margin-top:16px;font-size:13px;color:var(--tm);flex-wrap:wrap;}.r-stats strong{color:var(--text);font-weight:800;}.stat-item{background:rgba(255,255,255,.6);padding:6px 14px;border-radius:20px;}
.rc{max-width:740px;margin:0 auto;padding:0 20px 50px;}.rs{background:var(--card);border-radius:var(--radius);padding:24px;margin-bottom:18px;box-shadow:var(--shadow);}.rs-title{font-size:17px;font-weight:800;margin-bottom:18px;}
.level-card{text-align:center;padding:30px 20px;}.level-icon{font-size:64px;margin-bottom:8px;animation:floatY 3s ease-in-out infinite;}@keyframes floatY{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
...
.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;}.info-cell{padding:12px;background:#FAFBFF;border-radius:var(--rs);border:1px solid #d8d0e8;}.info-cell .ic-label{font-size:11px;color:var(--tl);font-weight:700;margin-bottom:2px;}.info-cell .ic-value{font-size:15px;font-weight:800;}.info-cell.highlight{background:var(--oxl);}.info-cell.highlight .ic-value{color:var(--o);}
...
.tag-list{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;}.tag-item{padding:5px 12px;border-radius:16px;font-size:12px;font-weight:700;}.tag-item.good{background:#EDE7F6;color:var(--o);}.tag-item.weak{background:var(--rl);color:var(--r);}
.plan-card{display:flex;gap:14px;padding:16px;background:var(--oxl);border-radius:var(--rs);}.plan-icon{font-size:32px;flex-shrink:0;}.plan-text h4{font-size:15px;font-weight:800;margin-bottom:4px;}.plan-text p{font-size:13px;color:var(--tm);line-height:1.7;}
.action-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;}.act-btn{padding:14px;border:2px solid #d0c8dc;border-radius:var(--rs);background:var(--card);font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;transition:var(--tr);display:flex;flex-direction:column;align-items:center;gap:4px;}.act-btn:hover{border-color:var(--o);color:var(--o);}.act-btn.primary{background:linear-gradient(135deg,var(--od),var(--o));color:#fff;border-color:transparent;}.act-btn .act-icon{font-size:20px;}.act-btn .act-label{font-size:12px;}
.rev-card{border:1.5px solid #d8d0e8;border-radius:var(--rs);margin-bottom:14px;overflow:hidden;}.rev-header{padding:14px 18px;background:#FAFBFF;display:flex;align-items:center;gap:12px;cursor:pointer;}.rev-header:hover{background:#F3F0FF;}.rev-icon{font-size:18px;margin-left:auto;transition:transform .2s;}.rev-body{max-height:0;overflow:hidden;transition:max-height .4s ease,padding .3s ease;padding:0 18px;}.rev-body.open{max-height:3000px;padding:20px 18px;}
.expl-tag{display:inline-flex;font-size:11px;font-weight:800;padding:4px 10px;border-radius:6px;margin-bottom:6px;}.expl-tag.ans{background:#EDE7F6;color:var(--o);}.expl-tag.tip{background:var(--oxl);color:var(--od);}
...
// ============================================================
// HELPER: Add 5th choice where needed
// ============================================================
function addFifthChoice(html) {
  // Match each question block to find ch arrays with < 5 items
  // Strategy: find ch:[...] blocks and check count
  // Handle both backtick and single-quote delimited items
  return html.replace(/ch:\[([^\]]*)\]/g, (match, inner) => {
    // Try backtick items first, then single-quote items
    let items = inner.match(/`[^`]*`/g) || [];
    let quoteType = '`';
    if (items.length === 0) {
      items = inner.match(/'[^']*'/g) || [];
      quoteType = "'";
    ...
    // Skip: T/F (2), written (0), already 5
    if (count === 0 || count === 2 || count >= 5) return match;

    const firstItem = items[0].replace(/[`']/g, '');

    // For ①②③ pattern (3 items) - make it ①②③④⑤
    if (count === 3 && firstItem === '①') {
      return `ch:[${quoteType}①${quoteType},${quoteType}②${quoteType},${quoteType}③${quoteType},${quoteType}④${quoteType},${quoteType}⑤${quoteType}]`;
    }

    // For ①②③④ pattern (4 items) - make it ①②③④⑤
    if (count === 4 && firstItem === '①') {
      return `ch:[${quoteType}①${quoteType},${quoteType}②${quoteType},${quoteType}③${quoteType},${quoteType}④${quoteType},${quoteType}⑤${quoteType}]`;
    }

    // For 4 English/Korean word choices - add a 5th distractor
    if (count === 4) {
      return match.slice(0, -1) + addDistractor(items, false, quoteType) + ']';
    }

    if (count === 3) {
      return match.slice(0, -1) + addDistractor(items, false, quoteType) + addDistractor(items, true, quoteType) + ']';
    }
...
function addDistractor(items, second = false, qt = '`') {
  // Try to infer what kind of choices these are
  const first = items[0].replace(/[`']/g, '');

  // Korean choices
  if (/[\uAC00-\uD7AF]/.test(first)) {
    if (second) return `,${qt}위 내용과 관련 없는 선지${qt}`;
    return `,${qt}위의 어느 것도 해당하지 않는다.${qt}`;
  }

  // English word choices
  if (second) return `,${qt}conclude${qt}`;
  return `,${qt}eliminate${qt}`;
}

// ============================================================
// HELPER: Fix ①②③ to ①②③④⑤ in underline-type questions
// ============================================================
function fixUnderlineQuestions(html) {
  // For 어법/어휘 questions with ①②③ (3 underlines),
  // we need to also fix the passage to have 5 underlines
  // This is complex - let's handle the simpler ch fix only
  // The passage changes would require content analysis
  return html;
}
...
function fixNewlines(html) {
  // In stem strings, replace literal \n with <br>
  // Handle backtick, single-quote, and double-quote delimited strings
  return html.replace(/(stem:[`'"])([^`'"]*?)([`'"])/g, (match, pre, content, post) => {
...
// ============================================================
// Build quiz HTML with slide UI
// ============================================================
function buildQuizHTML(origHTML, dirName) {
  // Extract data section (everything between first <script> and </script>)
  const scriptMatch = origHTML.match(/<script>([\s\S]*?)<\/script>/);
  if (!scriptMatch) {
    console.log('  WARNING: No script found in', dirName);
    return origHTML;
  }

  let dataSection = scriptMatch[1];

  // Extract EI
  const eiMatch = dataSection.match(/const EI=(\{.*?\});/s);
  if (!eiMatch) return origHTML;

  let ei;
  try {
    // Handle both JSON (double quotes) and JS object (single quotes)
    let eiStr = eiMatch[1].replace(/'/g, '"').replace(/\\"/g, "'");
    ei = JSON.parse(eiStr);
  } catch(e) {
    // Try eval as fallback
    try { ei = eval('(' + eiMatch[1] + ')'); } catch(e2) { return origHTML; }
  }

  // Extract everything from const EI to end of Q array
  const dataStart = dataSection.indexOf('const EI=');
  const qIdx = dataSection.indexOf('const Q=[');
  // Find the closing ]; after Q=[
  let qEnd = -1;
  if (qIdx !== -1) {
    // Look for ]; on its own line after Q=[
    qEnd = dataSection.indexOf('\n];\n', qIdx);
    if (qEnd === -1) qEnd = dataSection.indexOf('\n];', qIdx); // end of file case
  }
  if (qEnd === -1) return origHTML;

  const pureData = dataSection.substring(dataStart, qEnd + 3); // includes ];

  // Fix the data: add 5th choices, fix newlines
  let fixedData = addFifthChoice(pureData);
  fixedData = fixNewlines(fixedData);

  // Extract typeTag if present, otherwise build default
  const typeTagMatch = dataSection.match(/const typeTag=(\{[^}]+\});/);
  const typeTag = typeTagMatch ? typeTagMatch[0] :
    "const typeTag={'문장삽입':'tag-insert','어순배열':'tag-order','어법':'tag-grammar','어휘':'tag-vocab','빈칸추론':'tag-blank','서술형':'tag-written','내용이해':'tag-content','T/F':'tag-tf'};";

  // Build the full HTML with slide UI template
  return `<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<title>${ei.subject} ${ei.pub} 온라인퀴즈TEST</title>
...
<div class="start-container">
  <div class="brand-badge">📝 온라인퀴즈 TEST</div>
  <h1 class="start-title">온라인퀴즈 <span>TEST</span></h1>
  ...
  <div class="input-row">
    ...
    <div class="input-group" style="flex:.6"><label>학년</label><select id="inpGrade"><option>고1</option><option>고2</option><option selected>고3</option><option>중3</option></select></div>
  </div>
...
<div id="examScreen" class="screen">
  <div class="exam-topbar"><div class="topbar-left"><div class="topbar-dot"></div><div class="topbar-info">${ei.subject.replace('2025 고3 3월 모의고사','2025 고3 3월')} · <span>${ei.pub}</span></div></div><div class="timer" id="timer">20:00</div><button class="btn-submit" onclick="submitExam()">제출하기</button></div>
...
function toggleHistory(){const p=document.getElementById('historyPanel');if(p.style.display==='block'){p.style.display='none';return;}const h=getHistory();if(!h.length){p.innerHTML='<p style="text-align:center;color:var(--tl);font-size:13px;padding:20px">아직 기록이 없습니다.</p>';p.style.display='block';return;}p.innerHTML=h.map((x,idx)=>\`<div class="history-item" style="flex-wrap:wrap"><div class="hi-left"><div class="hi-name">\${x.name||'이름없음'}</div><div class="hi-meta">\${new Date(x.date).toLocaleDateString('ko-KR')} · \${x.grade} · \${x.level||''}</div></div><div class="hi-right"><div class="hi-score" style="color:\${x.score>=80?'var(--o)':'var(--r)'}">\${x.score}<span style="font-size:11px;color:var(--tl)">/\${x.total}</span></div></div>\${x.answers?\`<button onclick="reviewHistory(\${idx})" style="width:100%;margin-top:8px;padding:8px;border:1.5px solid #E0D8CF;border-radius:8px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;color:var(--o)">📝 정답·해설 다시 보기</button>\`:''}</div>\`).join('');p.style.display='block';}
function getLevel(s){if(s>=95)return{name:'마스터',icon:'👑',range:'95~100',msg:'이 단원은 정복했어요!'};if(s>=85)return{name:'챌린저',icon:'🌟',range:'85~94',msg:'거의 완벽, 한 끗 차이!'};if(s>=75)return{name:'파이터',icon:'💎',range:'75~84',msg:'통과! 약점만 잡으면 마스터'};if(s>=50)return{name:'루키',icon:'🔥',range:'50~74',msg:'기본기 OK, 유형별 훈련 필요'};return{name:'스타터',icon:'📖',range:'0~49',msg:'지문 해석부터 다시!'};}
function getTypeAdvice(t){const m={'문장삽입':'글의 흐름과 연결사 파악','어순배열':'문장 구조(S+V+O) 정확히 파악','어법':'핵심 어법 규칙 정리 및 반복 연습','어휘':'문맥에서 적절한 어휘 고르기 연습','빈칸추론':'빈칸 전후 문맥 단서 찾기','서술형':'정확한 철자와 어형 변환 주의','내용이해':'지문 핵심 내용 정확히 파악','T/F':'원문과 진술을 정확히 대조'};return m[t]||'해당 유형 집중 복습 필요';}
...
S.cur=idx;const q=Q[idx];const tc=typeTag[q.type]||'';const mk=q.ch&&q.ch.length===5?'①②③④⑤':q.ch&&q.ch.length===4?'①②③④':q.ch&&q.ch.length===3?'①②③':'①②';
...
function getTestName(){return '퀴즈(예상문제)테스트';}
function copyResult(){const r=grade(),lv=getLevel(r.score),pct=Math.round(r.score/EI.total*100),nm=document.getElementById('inpName').value.trim(),sc=document.getElementById('inpSchool').value.trim(),gr=document.getElementById('inpGrade').value,elapsed=Math.floor((Date.now()-S.start)/1000),pass=r.score>=80,tn=getTestName(),wk=getWeakTypes(),tm={};r.details.forEach(d=>{const t=d.q.type;if(!tm[t])tm[t]={total:0,ok:0};tm[t].total++;if(d.ok)tm[t].ok++;});let txt=\`📝 내신핏 \${tn} 결과\\n\${EI.subject} · \${EI.pub}\\n\${nm}\${sc?' · '+sc:''} · \${gr}\\n\${new Date().toLocaleDateString('ko-KR')}\\n\\n\${lv.icon} \${lv.name} · \${r.score}/\${EI.total}점 (\${pct}%)\\n\${pass?'🎉 통과!':'📚 미통과'} · ✅\${r.correct} ❌\${r.wrong} ⬜\${r.unanswered}\\n⏱️ \${fmtTime(elapsed)}\`;if(wk.length){txt+=\`\\n\\n🔍 내신핏 약점 진단\`;wk.forEach(w=>{const d=tm[w];txt+=\`\\n• \${w.replace(/\\(.+?\\)/g,'').trim()} \${d.ok}/\${d.total} → \${getTypeAdvice(w)}\`;});}txt+=\`\\n\\n📊 내신핏 | 내신 영어 완벽 대비\`;navigator.clipboard.writeText(txt).then(()=>toast('📋 결과가 복사되었습니다!')).catch(()=>{toast('복사 실패');});}
function shareImage(){const tn=getTestName(),r=grade(),lv=getLevel(r.score),pct=Math.round(r.score/EI.total*100),pass=r.score>=80,nm=document.getElementById('inpName').value.trim(),tm={};r.details.forEach(d=>{const t=d.q.type;if(!tm[t])tm[t]={total:0,ok:0};tm[t].total++;if(d.ok)tm[t].ok++;});const pc=pass?'#E8772E':'#D63B3B';let card=\`<div style="width:400px;font-family:-apple-system,sans-serif;background:#fff;border-radius:16px;overflow:hidden">\`;card+=\`<div style="background:linear-gradient(135deg,\${pass?'#FFF3E8,#FFE8D5':'#FFF0F0,#FFEAEA'});padding:28px 24px;text-align:center"><div style="font-size:12px;font-weight:800;color:\${pc};margin-bottom:4px">내신핏 \${tn}</div><div style="font-size:13px;color:#666;margin-bottom:16px">\${EI.subject} · \${EI.pub}</div><div style="font-size:56px;font-weight:900;color:#1A1A2E">\${r.score}<span style="font-size:20px;color:#aaa">/\${EI.total}</span></div><div style="margin-top:8px"><span style="display:inline-block;padding:6px 16px;border-radius:20px;font-size:14px;font-weight:800;background:\${pc};color:#fff">\${lv.icon} \${lv.name} · \${pass?'통과':'미통과'}</span></div><div style="margin-top:12px;font-size:12px;color:#888">👤 \${nm} · ✅\${r.correct} ❌\${r.wrong} ⬜\${r.unanswered}</div></div>\`;card+=\`<div style="padding:12px 20px;background:#f8f8f8;text-align:center;font-size:11px;color:#999">📊 <b style="color:#333">내신핏</b> | 내신 영어 완벽 대비</div></div>\`;const wrap=document.createElement('div');wrap.style.cssText='position:fixed;left:-9999px;top:0;';wrap.innerHTML=card;document.body.appendChild(wrap);html2canvas(wrap.firstChild,{scale:2,backgroundColor:null,useCORS:true}).then(canvas=>{document.body.removeChild(wrap);canvas.toBlob(blob=>{if(navigator.share&&blob){const file=new File([blob],'내신핏_퀴즈_결과.png',{type:'image/png'});navigator.share({files:[file],title:'내신핏 퀴즈 결과'}).catch(()=>{});}else{const a=document.createElement('a');a.href=canvas.toDataURL('image/png');a.download='내신핏_퀴즈_결과.png';a.click();}toast('📸 결과 이미지가 저장되었습니다!');});}).catch(()=>toast('이미지 생성 실패'));}
function reviewHistory(idx){const h=getHistory();const rec=h[idx];if(!rec||!rec.answers)return toast('이전 버전 기록은 해설을 볼 수 없습니다.');S.ans=rec.answers;S.start=Date.now()-rec.elapsed*1000;document.getElementById('inpName').value=rec.name||'';document.getElementById('inpSchool').value=rec.school||'';document.getElementById('inpGrade').value=rec.grade||'고3';const r=grade();renderResults(r,rec.elapsed);showScreen('resultScreen');window.scrollTo(0,0);}
...
// ============================================================
// Build word/workbook HTML with updated CSS + 5th choices
// ============================================================
function buildScrollHTML(origHTML, testType, dirName) {
  const css = testType === '단어테스트' ? WORD_CSS : WORKBOOK_CSS;
  const themeColor = testType === '단어테스트' ? 'g' : 'o';

  // Extract script section
  const scriptMatch = origHTML.match(/<script>([\s\S]*?)<\/script>/);
  if (!scriptMatch) return origHTML;

  let scriptContent = scriptMatch[1];

  // Extract EI
  const eiMatch = scriptContent.match(/const EI=(\{.*?\});/s);
  if (!eiMatch) return origHTML;
  let ei;
  try {
    let eiStr = eiMatch[1].replace(/'/g, '"').replace(/\\"/g, "'");
  ...
  // Fix data: add 5th choices, fix newlines
  scriptContent = addFifthChoice(scriptContent);
  scriptContent = fixNewlines(scriptContent);

  // Also need to fix the typeTag and renderQ to handle ①②③④⑤
  // Replace any hardcoded typeTag with a unified one

  const brandLabel = testType === '단어테스트' ? '온라인단어 TEST' : '온라인워크북 TEST';
  const shortLabel = testType === '단어테스트' ? '온라인단어' : '온라인워크북';
  const topInfo = `${ei.subject.replace('2025 고3 3월 모의고사','2025 고3 3월')} · <span>${ei.pub}</span>`;
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<title>${ei.subject} ${ei.pub} ${brandLabel}</title>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/variable/pretendardvariable-dynamic-subset.min.css">
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"><\/script>
<style>
${css}
...
<div id="startScreen" class="screen active">
<div class="start-container">
  <div class="brand-badge">📝 ${brandLabel}</div>
  <h1 class="start-title">${shortLabel} <span>TEST</span></h1>
  <p class="start-subtitle">${ei.subject} · ${ei.pub} · ${ei.lesson} · ${ei.totalQ}문항</p>
  <div class="info-card"><div class="info-card-header">📚 시험 정보</div><div class="info-rows">
    <div class="info-row"><span class="info-label">시험</span><span class="info-value">${ei.subject}</span></div>
    <div class="info-row"><span class="info-label">문항</span><span class="info-value">${ei.pub} (${ei.lesson})</span></div>
...
<div id="examScreen" class="screen">
  <div class="exam-topbar"><div class="topbar-left"><div class="topbar-dot"></div><div class="topbar-info">${topInfo}</div></div><div class="timer" id="timer">20:00</div><button class="btn-submit" onclick="submitExam()">제출하기</button></div>
  <div class="exam-scroll" id="examScroll"></div>
  <div class="exam-nav" id="navDots"></div>
</div>
<div id="resultScreen" class="screen"></div>
<script>
${scriptContent}
...
for (const dir of dirs.sort((a,b) => {
  ...
  for (const testFile of ['퀴즈테스트.html', '단어테스트.html', '워크북테스트.html']) {
    const filePath = path.join(dirPath, testFile);
    if (!fs.existsSync(filePath)) {
      issues.push(`${dir}/${testFile}: MISSING`);
      continue;
    }
    ...
    if (testFile === '퀴즈테스트.html') {
      newHTML = buildQuizHTML(origHTML, dir);
    } else {
      const testType = testFile.replace('.html', '');
      newHTML = buildScrollHTML(origHTML, testType, dir);
    }

    // Verify: count questions and total points
    const ids = (newHTML.match(/\{id:\d+/g) || []).length;
    const ptsAll = (newHTML.match(/pts:(\d+)/g) || []).map(p => parseInt(p.split(':')[1]));
    const total = ptsAll.reduce((a, b) => a + b, 0);

    if (ids !== 20) {
      issues.push(`${dir}/${testFile}: Q count=${ids} (expected 20)`);
    }
    if (total !== 100) {
      issues.push(`${dir}/${testFile}: total=${total} (expected 100)`);
    }
...
for (const dir of dirs) {
  ...
  for (const testFile of ['퀴즈테스트.html', '단어테스트.html', '워크북테스트.html']) {
    ...
    const hasH2c = html.includes('html2canvas');
    const hasSlide = html.includes('slideWrap');

    if (testFile === '퀴즈테스트.html' && (!hasH2c || !hasSlide)) {
      console.log(`${dir}/${testFile}: missing h2c=${!hasH2c} slide=${!hasSlide}`);
    }

    // Check remaining under-5 choices (handle both backtick and single-quote)
    const chBlocks = html.match(/ch:\[([^\]]*?)\]/g) || [];
    let under5 = 0;
    for (const cb of chBlocks) {
      let items = (cb.match(/`[^`]*`/g) || []);
      if (items.length === 0) items = (cb.match(/'[^']*'/g) || []);
      const cnt = items.length;
      if (cnt > 0 && cnt < 5 && cnt !== 2 && cnt !== 4) {
        under5++;
      }
    }
    if (under5 > 0) {
      console.log(`${dir}/${testFile}: still has ${under5} under-5 choice questions`);
    }
  }
}
